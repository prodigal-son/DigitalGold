...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: PayCon.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: PayCond.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 15394 or testnet: 15494)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 15395 or testnet: 15495)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePayConamount=<n> " + _("Keep N ROS2 anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PayCon is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PayCon is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PayCon version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeROS2Amount = GetArg("-anonymizeros2coinamount", 0);
    if(nAnonymizeROS2Amount > 999999) nAnonymizeROS2Amount = 999999;
    if(nAnonymizeROS2Amount < 2) nAnonymizeROS2Amount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeROS2Amount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize ROS2 Amount %d\n", nAnonymizeROS2Amount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ROS2+1000 == (.1ROS2+100)*10
       10ROS2+10000 == (1ROS2+1000)*10
    */
