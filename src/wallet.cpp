    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s CON %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s CON %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
// PayCon: total coins staked (non-spendable until maturity)
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nFeeRet < SOFT_MIN_TX_FEE && nChange > 0 && nChange < CENT)
                {
                    int64_t nMoveToFee = min(nChange, SOFT_MIN_TX_FEE - nFeeRet);
                    nChange -= nMoveToFee;
                    nFeeRet += nMoveToFee;
                }
		    ...
		    nChangePos = std::distance(wtxNew.vout.begin(), position);
                ...
                int64_t nMinFee = wtxNew.GetMinFee(1, GMF_SEND, nBytes, true); // true means that the soft fee increase is active here
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, targetProofOfStake))
            {
				...
				uint64_t nTotalSize = pcoin.first->vout[pcoin.second].nValue * (1+((txNew.nTime - block.GetBlockTime()) / (60*60*24)) * (MAX_MINT_PROOF_OF_STAKE2 / COIN / 365));
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nReward = GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, nFees);
        if (nReward <= 0)
            return false;
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, bool fAllowStakeForCharity)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, bool fAllowStakeForCharity)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, sNarr, wtxNew, fAskFee, fAllowStakeForCharity);
}
...
// PayCon: disable transaction (only for coinstake)
