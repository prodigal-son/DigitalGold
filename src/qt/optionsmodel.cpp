...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeROS2Amount"))
        settings.setValue("nAnonymizeROS2Amount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeROS2Amount = settings.value("nAnonymizeROS2Amount").toLongLong();
    ...
    if (settings.contains("nAnonymizeROS2Amount"))
        SoftSetArg("-anonymizePayConamount", settings.value("nAnonymizeROS2Amount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeROS2Amount:
            nAnonymizeROS2Amount = value.toInt();
            settings.setValue("nAnonymizeROS2Amount", nAnonymizeROS2Amount);
            emit anonymizeROS2AmountChanged(nAnonymizeROS2Amount);
            break;
        default:
            break;
        }
