...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PayCon.lnk";
}
...
{
    return GetAutostartDir() / "PayCon.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream PayConFile(GetAutostartFilePath());
    if (!PayConFile.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!PayConFile.eof())
    {
        getline(PayConFile, line);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream PayConFile(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!PayConFile.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        PayConFile << "[Desktop Entry]\n";
        PayConFile << "Type=Application\n";
        PayConFile << "Name=PayCon\n";
        PayConFile << "Exec=" << pszExePath << " -min\n";
        PayConFile << "Terminal=false\n";
        PayConFile << "Hidden=false\n";
        PayConFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PayCon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PayCon-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(48,201,204); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(0,182,186); border-left-style: solid; border-left-width: 6px; }"
